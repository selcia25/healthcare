...
# List of words
word_list = ['PROTEIN', 'ANTIBODY', 'MOLECULE', 'DIAGNOSIS', 'TREATMENT', 'ALLERGY', 'RESEARCH', 'MEDICINE', 'ANALYSIS', 'HOSPITAL']
...
def show_game_outcome_message(outcome):
    global exit_game  # Use the global variable
    ...
    ok_button = tk.Button(custom_box, text="OK", command=lambda: [custom_box.destroy(), root.destroy()], font=("Arial", 14), bg="white")
    ...
    custom_box.protocol("WM_DELETE_WINDOW", lambda: setattr(exit_game, 'exit_game', True))
    custom_box.mainloop()
